...
static void test_parse_success(void **)
{
	...
	exp = std::vector<uint8_t>(
		{0xF0, 0x41, 0x40, 0x00, 0x00, 0x24, 0x11, 0x05, 0x06, 0x07, 0x08, 0x40, 0x30, 0x20, 0x10});
	add_midi_sum_eox(exp, 7);
	assert_vector_equal(pkt.midi, exp);
}
...
static void test_add_from_raw(void **)
{
	...
	v = std::vector<uint8_t>(
		{0x02, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0xA4, 0x00, 0x00, 0x00, 0xB7, 0x00, 0x00, 0x00});
	assert_int_equal(pkt.add_from_raw(&v[0], v.size()), 16);

	// Error: expecting MIDI data end but length mismatch
	v = std::vector<uint8_t>(
		{0x02, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x07, 0x0a, 0x0b, 0x0c, 0x04, 0x0d, 0x0e, 0x0f});
	pkt.midi.clear();
	assert_int_equal(pkt.add_from_raw(&v[0], v.size()), 16);
	...
	v = std::vector<uint8_t>(
		{0x02, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
	assert_int_equal(pkt.add_from_raw(&v[0], v.size()), 16);
}
...
static void test_parse_tcp_stream(void **)
{
	...
	v = std::vector<uint8_t>({0x02, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00,
				  0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00});
	pkt.midi.clear();
	assert_int_equal(parse_tcp_stream(&pkt, &v[0], v.size()), v.size());

	// is_dt1 returns true but EOX was not found
	v = std::vector<uint8_t>({0x02, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x04, 0xF0,
				  0x41, 0x00, 0x04, 0x00, 0x00, 0x00, 0x07, 0x12, 0x00, 0x00});
	pkt.midi.clear();
	assert_int_equal(parse_tcp_stream(&pkt, &v[0], v.size()), 0);
	...
	// is_dt1 returns true
	v = std::vector<uint8_t>({0x02, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x04, 0xF0,
				  0x41, 0x00, 0x04, 0x00, 0x00, 0x00, 0x07, 0x12, 0x00, 0xF7});
	pkt.midi.clear();
	assert_int_equal(parse_tcp_stream(&pkt, &v[0], v.size()), v.size());
...
int main()
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test(test_parse_success),    cmocka_unit_test(test_parse_failures),
		cmocka_unit_test(test_vmxpacket_h),      cmocka_unit_test(test_add_from_raw),
